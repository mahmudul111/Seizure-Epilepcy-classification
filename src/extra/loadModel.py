...
#     os.path.join(project_root, "extracted_models", "saved_models"),
#     "saved_models",
#     "extracted_models/saved_models",
# ]
...
#     input_shape_list = config_dict["config"]["layers"][0]["config"]["batch_input_shape"][1:]
#     input_shape = tuple(input_shape_list)

#     last_layer = config_dict["config"]["layers"][-1]
...
# print("\nRebuilding model architecture...")
# model = build_cnn_attention_model(input_shape=input_shape, num_classes=num_classes)
...
#                     # Extract output - get the first value from output dict
#                     result = list(output.values())[0]
#                     return result.numpy()
...
#             # Replace model with wrapper
#             model = SavedModelWrapper(infer, input_shape, num_classes)
#             weights_loaded = True

#         except Exception as e2:
#             print(f"Failed to load from SavedModel: {str(e2)}")
#             raise FileNotFoundError("Could not load model from any format")
#     else:
#         raise FileNotFoundError(f"SavedModel directory not found: {savedmodel_path}")
...
# # ============================================
# def preprocess_data(data):
#     """Preprocess your input data - UPDATE THIS based on your preprocessing"""
#     # Example: normalize if you normalized during training
#     # data = (data - mean) / std
#     return data
...
#     all_probabilities = model.predict(data_batch, verbose=0)
#     predictions = np.argmax(all_probabilities, axis=1)
#     confidences = np.max(all_probabilities, axis=1)
...
# dummy_sample = np.random.randn(*input_shape)
# dummy_batch = np.random.randn(5, *input_shape)

# # Single prediction
# print("\nTesting single sample prediction...")
# predicted_class, confidence, probs = predict_single_sample(model, dummy_sample)
# print(f"✓ Predicted Class: {predicted_class}")
# print(f"✓ Confidence: {confidence:.4f}")
# print(f"✓ All Probabilities: {probs}")
...
# # Batch prediction
# print("\nTesting batch prediction...")
# predictions, confidences, _ = predict_batch(model, dummy_batch)
# print(f"✓ Predictions: {predictions}")
# print(f"✓ Confidences: {confidences}")
...
# print("1. Load your data: X_test = np.load('your_data.npy')")
# print("2. Preprocess: X_test = preprocess_data(X_test)")
# print("3. Predict: predictions, confidences, _ = predict_batch(model, X_test)")
